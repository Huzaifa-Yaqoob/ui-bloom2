export const tsxCode = `'use client';
...
export const jsxCode = `'use client';
...
function LogicalPaginationComponent(props) {
  const {
    size,
    totalPages,
    currentPage,
  ...
  const visiblePages = useMemo(() => {
    ...
    const getClosestSize = (...keys) => {
      for (const key of keys) {
        if (size?.[key] != null) return size[key];
      }
      return 5;
    };
...
const DefaultPageNumber = ({ num, isActive, ...props }) => (
  <Button variant={isActive ? 'default' : 'outline'} size="sm" {...props}>
    {num}
  </Button>
);

const DefaultGapPlaceholder = (props) => (
  <Button variant="ghost" size="sm" disabled {...props}>
    <Ellipsis />
  </Button>
);

const DefaultPrevious = (props) => (
  <Button variant="outline" size="sm" {...props}>
    <ChevronLeft />
  </Button>
);

const DefaultNext = (props) => (
  <Button variant="outline" size="sm" {...props}>
    <ChevronRight />
  </Button>
);
